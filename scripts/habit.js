...
// Run check on page load
    // (Will be called from init() once default habits are loaded)

/*
   Ensure app has a known habit dataset for empty localStorage (GitHub live origin has empty storage).
*/
async function loadInitialHabits() {
    const currentHabits = JSON.parse(localStorage.getItem('habit')) || [];
    ...
    try {
        ...
        console.warn('No default habits loaded (habit.json not available or invalid):', err.message);
    }
}
